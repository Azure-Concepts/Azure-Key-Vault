...
namespace LMS.Data.Repository
{
    public class BookRepository: IBookRespository
    {
        ...
        {
            var bookEntities = _libraryDbContext.Books
                .Include(bk => bk.Author);

            return bookEntities.Select(bookEntity =>
                new Book()
                {
                    Id = bookEntity.BookId,
                    ISBN = bookEntity.ISBN,
        ...
        public Book GetCheckoutHistory(Book book)
        {
            var checkoutHistory = _tableService.GetAsync(1000, $"PartitionKey eq '{book.Title}'").Result;
            book.CheckoutHistory = checkoutHistory.Results.Select(ch => new CheckoutRecord()
            {
                CheckoutBy = new User() { Name = ch.UserAlias },
                CheckoutOn = ch.CheckoutOn,
                ReturnedOn = ch.ReturnOn
            }).ToArray();
            return book;
        }
        ...
        public Book GetBook(string bookTitle)
        {
            ...
            var authorEntity = _libraryDbContext.Authors.FirstOrDefault(ath => ath.AuthorId == bookEntity.AuthorId);

            return new Book()
            {
                Id = bookEntity.BookId,
                ISBN = bookEntity.ISBN,
                Author = authorEntity.AuthorName,
                Title = bookEntity.BookName,
                PublisherName = bookEntity.Publisher
            };
        }
